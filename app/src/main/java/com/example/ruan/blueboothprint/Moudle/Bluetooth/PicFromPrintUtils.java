...
//        Gray = 0.29900 * R + 0.58700 * G + 0.11400 * B
     ...
     *
     *
     * 注意这里的打印需要配置打印机的设置才能正常打印
     * 比如说我测试的zicox打印机的 打印密度为203 ，打印点为24x24
     *
     * @param bit
     * param width
     * param height
     * @return
     */
    public static byte[] draw2PxPoint(Bitmap bit , int width , int height) {

        //在进行打印之前先把图片压缩成想要的大小
        Bitmap bitmap = compressPic(bit , width , height);

        //因为是24x24所以将高低分成height / 24 组来进行打印
        //所以每组要打印 24 * width
        //由于每8个字节为一个像素点所以就是3 * 8

        int dataLength = height / 24 * 3 * width + 8 * height / 24;
        Log.e("Ruan" , dataLength + "---" + height / 24);
